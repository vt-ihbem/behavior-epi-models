...
IC = [s0, e0, i0, r0, f0, zeros(1,20)*10e-5];
...
h1=figure(1)
plot(t,y(:,8),'r',t,y(:,13),'--r',t,y(:,18),'-.r',t,y(:,23),':r','LineWidth',2) 
xlim([0,tmax]);
ylabel('Sensitivity','FontSize',24)
xlabel('Time (days)','FontSize',24)
legend('$i_{\beta}$','$i_a$',...
        '$i_{\gamma}$','$i_{\tau_R}$',...
        'interpreter','latex','FontSize',22,'NumColumns',2)
legend boxoff
ax = gca;
ax.XAxis.FontSize = 24;
ax.YAxis.FontSize = 24;
saveas(h1,'Sens_I_SEIRSb.png');

%%%%%%%% Semi-relative (SR) sensitivity

h2=figure(2)
plot(t,p.beta*y(:,8),'r',t,p.a*y(:,13),'--r',t,p.gamma*y(:,18),'-.r',t,p.tauR*y(:,23),':r','LineWidth',2)
xlim([0,tmax]);
ylabel('Semi-relative sensitivity','FontSize',24)
xlabel('Time (days)','FontSize',24)
legend('$\beta i_{\beta}$','$a i_a$',...
        '$\gamma i_{\gamma}$','$\tau_R i_{\tau_R}$',...
        'interpreter','latex','FontSize',22,'NumColumns',2)
legend boxoff
ax = gca;
ax.XAxis.FontSize = 24;
ax.YAxis.FontSize = 24;
saveas(h2,'SR_Sens_I_SEIRSb.png');

%%%%%%% Graph infectious population sol'n
...
s_tauR=y(21);   e_tauR=y(22);   i_tauR=y(23);   r_tauR=y(24);   f_tauR=y(25);
dy = zeros(25,1);
...
dy(6) = -1/((1+p.a*f)^p.gamma)*s*i-p.beta/((1+p.a*f)^p.gamma)*i*s_beta-p.beta/((1+p.a*f)^p.gamma)*s*i_beta...
            +r_beta/p.tauR+(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_beta;
...
dy(9) = i_beta/p.tauI-r_beta/p.tauR;
...
dy(11) =  (p.beta*p.gamma*f)/((1+p.a*f)^(p.gamma+1))*s*i-p.beta/((1+p.a*f)^(p.gamma))*i*s_a...
            -p.beta/((1+p.a*f)^(p.gamma))*s*i_a+r_a/p.tauR+(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_a;
dy(12) = -(p.beta*p.gamma*f)/((1+p.a*f)^(p.gamma+1))*s*i+p.beta/((1+p.a*f)^(p.gamma))*i*s_a...
           -(1/p.tauE)*e_a+p.beta/((1+p.a*f)^p.gamma)*s*i_a-(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_a;
dy(13) = e_a/p.tauE-i_a/p.tauI;
dy(14) = i_a/p.tauI-r_a/p.tauR;
...
dy(16) =  p.beta/((1+p.a*f)^(p.gamma))*s*i*log(1+p.a*f)-p.beta/((1+p.a*f)^(p.gamma))*i*s_gamma...
            -p.beta/((1+p.a*f)^(p.gamma))*s*i_gamma+r_gamma/p.tauR+(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_gamma;
...
dy(19) = i_gamma/p.tauI-r_gamma/p.tauR;
dy(20) = (i_gamma-f_gamma)/p.tauF;

% Partial derivatives of S, E, I, R, F wrt tauR

dy(21) = -r/p.tauR^2 - p.beta/((1+p.a*f)^(p.gamma))*i*s_tauR-p.beta/((1+p.a*f)^(p.gamma))*s*i_tauR...
            +r_tauR/p.tauR+(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_tauR;
dy(22) = p.beta/((1+p.a*f)^(p.gamma))*i*s_tauR-e_tauR/p.tauE+p.beta/((1+p.a*f)^(p.gamma))*s*i_tauR...
            -(p.a*p.beta*p.gamma)/((1+p.a*f)^(p.gamma+1))*s*i*f_tauR;
dy(23) = e_tauR/p.tauE-i_tauR/p.tauI;
dy(24) = r/p.tauR^2 + i_tauR/p.tauI-r_tauR/p.tauR;
dy(25) = (i_tauR-f_tauR)/p.tauF;
end
