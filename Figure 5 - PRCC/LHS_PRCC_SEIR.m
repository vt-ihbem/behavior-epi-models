...
% LHS_raw: {'\alpha','\beta','\gamma','\tau_E','\tau_I','\tau_F','N','\tau_R'};
...
for j=1:size1 
    ...
    [t,y]=ode45(@SEIR, 0:1:tmax, IC, options1, par);
...
sgtitle('SEIR: Max i - Check for monotonic relationships')
saveas(h1,'Mono_QOI_1_SEIR.png')
...
sgtitle('SEIR: i at 50 days - Check for monotonic relationships')
saveas(h11,'Mono_QOI_2_SEIR.png')
...
sgtitle('SEIR: i at 2 years - Check for monotonic relationships')
saveas(h21,'Mono_QOI_3_SEIR.png')
...
% Sort elements in order of SEIRb: tauI, beta, tauE
K=[2 3 1];
[~,sortOrder] = sort(K);
...
ylabel('PRCC')
ylim([-1 1])
%title('SEIR: maximum size of infectious population') 
...
saveas(h2,'PRCC_QOI_1_SEIR.png')
saveas(h2,'PRCC_QOI_1_SEIR.fig')
saveas(h2,'PRCC_QOI_1_SEIR.pdf')
...
ylabel('PRCC')
ylim([-1 1])
%title('SEIR: infectious population size at 50 days')
...
saveas(h12,'PRCC_QOI_2_SEIR.png')
saveas(h12,'PRCC_QOI_2_SEIR.fig')
saveas(h12,'PRCC_QOI_2_SEIR.pdf')
...
ylabel('PRCC')
ylim([-1 1])
%title('SEIR: infectious population size at 2 years')
...
saveas(h22,'PRCC_QOI_3_SEIR.png')
saveas(h22,'PRCC_QOI_3_SEIR.fig')
saveas(h22,'PRCC_QOI_3_SEIR.pdf')

save("LHS_PRCC_SEIR_output.mat",...
     'sortedPRCC_1','sortedPRCC_2','sortedPRCC_3','sortedstat_p1',...
     'sortedstat_p2','sortedstat_p3','sortedparams','pars')

%% Function

function dy = SEIR(~,y,p)
